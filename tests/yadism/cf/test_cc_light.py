...
class Test_Blumlein_results:
    ...
    def test_f3_bluemlein(self):
        f3_ns_ref = np.array(
            [
                [-25231.429850963934, 1541.529626144562, -547.2207168022869],
                [-10211.246584770022, 1542.2108232096089, -547.2207168023251],
                [-2080.6483437193, 1549.033985539169, -547.2207168023015],
                [-1582.042993312226, 1618.3994999557885, -547.2207168022896],
                [-3137.5438037392723, 2442.110327324382, -547.2207168023087],
                [-4369.890283850198, 3726.627423809562, -547.2207168023033],
                [-6023.31882637291, 5612.869667249121, -547.2207168022851],
                [-18980.411027186972, 70966.81668869738, -547.2207168022705],
            ]
        )
        ...
        # TODO: why Vogt seems to neglect the d33 piece?
        # ns,+
        # np.testing.assert_allclose(
        #     np.array(f3_ns_result)[:, :-1],
        #     (f3_ns_ref + self.w2 * f3_d33_ref)[:, :-1],
        # )

        # ns,+
        np.testing.assert_allclose(
            np.array(f3_ns_result)[:, :-1], (f3_ns_ref)[:, :-1], rtol=3e-4
        )
