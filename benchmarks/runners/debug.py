...
def analyze_fhat():
    ...
    def yadf1hat(efhat):
        ...
        pc.init_nlo_vars()
        return efhat["Splus"] * ic.f1_splus_raw(pc) + efhat["Sminus"] * ic.f1_sminus_raw(pc)
    def yadf2hat(efhat):
        esf.x = 0.1
        esf.Q2 = efhat["Q2"]
        pc = PartonicChannelHeavyIntrinsic(esf,efhat["m1sq"],efhat["m2sq"])
        pc.init_vars(efhat["z"])
        pc.init_nlo_vars()
        return efhat["Splus"] * ic.f2_splus_raw(pc) + efhat["Sminus"] * ic.f2_sminus_raw(pc)
    ...
    def apf_ratio(n1, n2):
        ...
        print(f"z1: {f2hat['z'][n1]:.3f} - z2: {f2hat['z'][n2]:.3f} --- Q2: {f2hat['Q2'][n1]:.3f}")
        f2 = f2hat["apf"]
        f1 = f1hat["apf"]
