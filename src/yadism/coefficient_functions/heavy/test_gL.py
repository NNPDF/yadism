...
def test_cg_NNLO():
    ...
    for nf in [3, 4]:
        for z in [1e-1, 1e-2, 1e-3]:
            ...
            a_1 = order(cg_1).reg(z, order(cg_1).args["reg"]) #sing and loc the same (?)
            a_2 = order(cg_2).reg(z, order(cg_2).args["reg"])
            np.testing.assert_allclose(a_1,- a_2, err_msg='gL and FL coefficients at NNLO should be the same, but are not')