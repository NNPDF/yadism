...
def f2_splus_raw(pc):
    return (pc.s1hat*(-2*(pc.m1sq + pc.m2sq)*pc.s1hat**2 - 2*(pc.delta**2 - 6*pc.m1sq*pc.Q2)*(pc.m2sq + pc.s1hat) - (2*pc.delta**4*(2*pc.m2sq + pc.s1hat))/pc.s1hat**2 - 9*pc.m2sq*pc.sigma_pm**2 + 2*pc.s1hat*(2*pc.delta**2 + (pc.m1sq - 5*pc.m2sq)*pc.sigma_pm) - (2*pc.delta**2*(pc.delta**2 + 2*(2*pc.m2sq + pc.s1hat)*pc.sigma_pm))/pc.s1hat - pc.delta**2*(pc.m2sq - 2*pc.sigma_pp) + ((pc.deltap**2 - 6*pc.Q2*(pc.m2sq + pc.s1hat))*pc.sigma_pp*(pc.s1hat + pc.sigma_pp))/(2.*(pc.m2sq + pc.s1hat))))/(pc.deltap**2*pc.Q2*(pc.m2sq + pc.s1hat)) + (pc.L_xi*(-2*pc.delta**4*(pc.s1hat + pc.sigma_pp) - 2*pc.delta**2*pc.s1hat*(pc.s1hat + 2*pc.sigma_pm)*(pc.s1hat + pc.sigma_pp) + pc.s1hat**2*(-(pc.deltap**2*pc.sigma_pp) + 6*pc.m1sq*pc.Q2*(2*pc.s1hat + 3*pc.sigma_pp))))/(pc.deltap**3*pc.Q2*pc.s1hat)

def f2_splus_soft(pc):
    return (-2*(2*pc.delta + pc.L_xisoft*pc.sigma_pp))/pc.Q2

def f2_splus_virt(pc):
    return (pc.delta*(2*pc.CRm + pc.C1p*pc.m1sq + pc.C1m*pc.m2sq))/(2.*pc.Q2)

def f2_sminus_raw(pc):
    return (2*pc.L_xi*np.sqrt(pc.m1sq * pc.m2sq)*(pc.deltap**2 - 6*pc.m1sq*pc.Q2)*pc.s1hat)/(pc.deltap**3*pc.Q2) - (np.sqrt(pc.m1sq * pc.m2sq)*pc.s1hat*(pc.deltap**2*(-4*pc.m2sq - 3*pc.s1hat + pc.sigma_pp) + 6*pc.Q2*(pc.m2sq + pc.s1hat)*(pc.s1hat + pc.sigma_pp)))/(pc.deltap**2*pc.Q2*(pc.m2sq + pc.s1hat)**2)

def f2_sminus_soft(pc):
    return 0

def f2_sminus_virt(pc):
    return (pc.delta*(2*pc.Cplus + (pc.C1m + pc.C1p)*np.sqrt(pc.m1sq * pc.m2sq)))/(2.*pc.Q2)

def fl_splus_raw(pc):
    return (-8*pc.delta**4*(pc.m2sq + pc.s1hat)**2 + 2*pc.delta**2*(pc.s1hat*(pc.m2sq + pc.s1hat)*(-(pc.m2sq*(3*pc.s1hat + 8*pc.sigma_pm)) + 2*pc.s1hat*(pc.s1hat - 2*pc.sigma_pm + pc.sigma_pp)) + pc.Q2*(9*pc.s1hat**3 + 4*pc.m2sq**2*pc.sigma_pp + 2*pc.s1hat*(4*pc.m2sq + pc.s1hat)*pc.sigma_pp)*pc.x) + pc.s1hat*(pc.s1hat*(-2*(pc.m2sq + pc.s1hat)*(pc.m2sq*(2*pc.s1hat**2 + 10*pc.s1hat*pc.sigma_pm + 9*pc.sigma_pm**2) - 2*pc.m1sq*(6*pc.m2sq*pc.Q2 + pc.s1hat*(6*pc.Q2 - pc.s1hat + pc.sigma_pm))) + pc.s1hat*(pc.deltap**2 - 6*pc.Q2*(pc.m2sq + pc.s1hat))*pc.sigma_pp + (pc.deltap**2 - 6*pc.Q2*(pc.m2sq + pc.s1hat))*pc.sigma_pp**2) - pc.Q2*(pc.s1hat*(28*pc.m1sq**2*pc.s1hat + pc.s1hat**3 + 2*pc.m1sq*pc.s1hat*(-8*pc.m2sq + 14*pc.Q2 + 3*pc.s1hat) - 3*pc.s1hat**2*pc.sigma_pm + 4*pc.m2sq*pc.sigma_mp*pc.sigma_pm) + (4*pc.m1sq*pc.m2sq*pc.s1hat + 7*pc.s1hat**2*pc.sigma_mp - 8*pc.m2sq*(2*pc.m2sq + 5*pc.s1hat)*pc.sigma_pm)*pc.sigma_pp - 3*pc.s1hat*pc.sigma_mp*pc.sigma_pp**2)*pc.x))/(2.*pc.deltap**2*pc.Q2*pc.s1hat*(pc.m2sq + pc.s1hat)**2) + (pc.L_xi*(-2*pc.delta**4*(pc.s1hat + pc.sigma_pp) - 2*pc.delta**2*(pc.s1hat + pc.sigma_pp)*(pc.s1hat**2 + 2*pc.s1hat*pc.sigma_pm - pc.Q2*pc.sigma_pp*pc.x) + pc.s1hat*(-(pc.deltap**2*pc.s1hat*pc.sigma_pp) + pc.Q2*(pc.s1hat**3 + 4*pc.s1hat**2*pc.sigma_pm + 7*pc.s1hat*pc.sigma_pm*pc.sigma_pp + 4*pc.sigma_pm*pc.sigma_pp**2)*pc.x + 2*pc.m1sq*pc.Q2*pc.s1hat*(6*pc.s1hat + 9*pc.sigma_pp - 2*pc.m2sq*pc.x))))/(pc.deltap**3*pc.Q2*pc.s1hat)
...
def fl_splus_virt(pc):
    return (pc.delta*(2*pc.CRm + pc.C1p*pc.m1sq + pc.C1m*pc.m2sq))/(2.*pc.Q2) + ((2*pc.Cplus*np.sqrt(pc.m1sq * pc.m2sq) - pc.CRm*pc.sigma_pp)*pc.x)/pc.delta

def fl_sminus_raw(pc):
    return (2*pc.L_xi*np.sqrt(pc.m1sq * pc.m2sq)*((pc.deltap**2 - 6*pc.m1sq*pc.Q2)*pc.s1hat**2 - 2*pc.Q2*(pc.delta**2*pc.sigma_pp + pc.s1hat*(pc.deltap**2 + (pc.m2sq + pc.Q2)*pc.s1hat + 2*pc.sigma_pm*pc.sigma_pp))*pc.x))/(pc.deltap**3*pc.Q2*pc.s1hat) + (np.sqrt(pc.m1sq * pc.m2sq)*(-(pc.s1hat**2*(pc.deltap**2*(-4*pc.m2sq - 3*pc.s1hat + pc.sigma_pp) + 6*pc.Q2*(pc.m2sq + pc.s1hat)*(pc.s1hat + pc.sigma_pp))) - 2*pc.Q2*(pc.m2sq + pc.s1hat)*(2*pc.delta**2*(2*pc.m2sq + pc.s1hat) + pc.s1hat*(2*pc.deltap**2 + 8*pc.m2sq*pc.sigma_pm + pc.s1hat*(pc.s1hat - 2*pc.sigma_mp + 4*pc.sigma_pm + pc.sigma_pp)))*pc.x))/(pc.deltap**2*pc.Q2*pc.s1hat*(pc.m2sq + pc.s1hat)**2)
...
def fl_sminus_virt(pc):
    return (2*pc.Cplus*pc.delta + (pc.C1m + pc.C1p)*pc.delta*np.sqrt(pc.m1sq * pc.m2sq))/(2.*pc.Q2) + ((2*pc.CRm*np.sqrt(pc.m1sq * pc.m2sq) - pc.Cplus*pc.sigma_pp)*pc.x)/pc.delta
